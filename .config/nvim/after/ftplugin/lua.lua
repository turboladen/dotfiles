...
vim.wo.foldexpr = "v:lnum==1?'>1':getline(v:lnum)=~'^\\s*--'&&getline(v:lnum+1)=~'^\\s*[^-]'?'>1':getline(v:lnum)=~'^\\s*}'?'<1':getline(v:lnum)=~'^\\s*{'?'>1':'='"
...
map("<leader>lm", function()
  ...
  if lua_path then
    ...
    if ok then
      ...
      print("Error reloading module: " .. tostring(result))
    end
  else
    print("Not a Lua module in the lua/ directory")
  end
end, "Reload [M]odule")
...
map("<leader>li", function()
  ...
  local inspect_call = string.format("print(vim.inspect(%s))", word)
